...
function ProductMediaGallery(props) {
  ...
  const { refetchProduct,product,updateProduct } = useProduct();
  ...
  const handleRemoveMedia = async () => {
    console.log("mediaItemToRemove",mediaItemToRemove)
    let updatedMedia=media.filter(mediaItem=>mediaItem?.URLs.small!==mediaItemToRemove?.URLs.small);
    console.log("updatedMedia",updatedMedia)
    
    Meteor.call("admin/updateProduct",{productId:product._id,field:"media",value:updatedMedia})
